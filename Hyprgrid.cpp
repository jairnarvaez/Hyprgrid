...
int CHyprgrid::getAdjacentWorkspaceID(eHyprgridDirection direction) {
    ...
    switch (direction) {
        case HYPRGRID_LEFT:
            return workspaceIDLeft;
        case HYPRGRID_RIGHT:
            return workspaceIDRight;
        case HYPRGRID_UP:
            return workspaceIDUp;
        case HYPRGRID_DOWN:
            return workspaceIDDown;
        default:
            return WORKSPACE_INVALID;
    }
}

void CHyprgrid::handleGesture(const ITrackpadGesture::STrackpadGestureUpdate& e, 
    ...
    if (!hyprgrid_enable_wrap_around && hitBorder) {
        m_delta = 0;
    ...
    bool isInvalidMove = (!PWORKSPACE) || 
                        (m_delta < 0 && targetWorkspaceID > m_workspaceBegin->m_id) || 
                        (m_delta >= 0 && targetWorkspaceID < m_workspaceBegin->m_id);
    
    if (!hyprgrid_enable_wrap_around && isInvalidMove) {
        m_delta = 0;
        return;
    }
...
void CHyprgrid::finalizeGesture(const ITrackpadGesture::STrackpadGestureEnd& e, 
                                 int workspaceIDLeft, int workspaceIDRight, 
                                 int workspaceIDUp, int workspaceIDDown) {
    ...
    bool shouldCancel = (abs(m_delta) < *m_swipeDistance * *m_swipeCancelRatio && 
                        (*m_swipeMinSpeedToForce == 0 || 
                         (*m_swipeMinSpeedToForce != 0 && m_avgSpeed < *m_swipeMinSpeedToForce))) || 
                        abs(m_delta) < 2 || hitBorder;
    ...
    if (shouldCancel) {
        ...
        *m_workspaceBegin->m_renderOffset = Vector2D();
        ...
        PHLWORKSPACE pTargetWorkspace = m_vertanim ? 
                                        (isMovingUpOrLeft ? PWORKSPACEU : PWORKSPACED) : 
                                        (isMovingUpOrLeft ? PWORKSPACEL : PWORKSPACER);
        int targetWorkspaceID = m_vertanim ? 
                               (isMovingUpOrLeft ? workspaceIDUp : workspaceIDDown) : 
                               (isMovingUpOrLeft ? workspaceIDLeft : workspaceIDRight);
        ...
        *m_workspaceBegin->m_renderOffset = finalBeginOffset;
        ...
        const char* logMessage = m_vertanim ? 
                                (isMovingUpOrLeft ? "Ended swipe UP" : "Ended swipe DOWN") : 
                                (isMovingUpOrLeft ? "Ended swipe to the left" : "Ended swipe to the right");
...
void CHyprgrid::update(const ITrackpadGesture::STrackpadGestureUpdate& e) {
    ...
    if (workspaceIDLeft == WORKSPACE_INVALID || workspaceIDRight == WORKSPACE_INVALID || 
        workspaceIDUp == WORKSPACE_INVALID || workspaceIDDown == WORKSPACE_INVALID || 
        workspaceIDLeft == m_workspaceBegin->m_id || workspaceIDRight == m_workspaceBegin->m_id || 
        workspaceIDUp == m_workspaceBegin->m_id || workspaceIDDown == m_workspaceBegin->m_id) {
        m_workspaceBegin = nullptr;
        HyprlandAPI::addNotification(PHANDLE, "Workspace invalido", { 1.0, 0.2, 0.2, 1.0 }, 5000);
        return;
    }
    
    handleGesture(e, workspaceIDLeft, workspaceIDRight, workspaceIDUp, workspaceIDDown);
}
