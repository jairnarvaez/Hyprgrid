...
bool CHyprgrid::isAtBoundary(int workspaceID, eHyprgridDirection direction)
{
    switch (direction) {
    case HYPRGRID_LEFT:
        return (workspaceID % hyprgrid_grid_size_x == 1);
    case HYPRGRID_RIGHT:
        return (workspaceID % hyprgrid_grid_size_x == 0);
    case HYPRGRID_UP:
        return (workspaceID <= hyprgrid_grid_size_x);
    case HYPRGRID_DOWN:
        return (workspaceID > (hyprgrid_grid_size_x * (hyprgrid_grid_size_y - 1)));
    default:
        return false;
    }
}
    ...
    int workspaceIDUp, int workspaceIDDown)
{
    ...
    bool hitLeftBorder = isAtBoundary(m_workspaceBegin->m_id, HYPRGRID_LEFT) && m_delta < 0 && !m_vertanim;
    bool hitRightBorder = isAtBoundary(m_workspaceBegin->m_id, HYPRGRID_RIGHT) && m_delta > 0 && !m_vertanim;
    bool hitTopBorder = isAtBoundary(m_workspaceBegin->m_id, HYPRGRID_UP) && m_delta < 0 && m_vertanim;
    bool hitBottomBorder = isAtBoundary(m_workspaceBegin->m_id, HYPRGRID_DOWN) && m_delta > 0 && m_vertanim;
